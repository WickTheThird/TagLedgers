<script lang="ts">
	import { page } from '$app/stores';

	let urlError = $derived($page.url.searchParams.get('error'));

	const errorMessages: Record<string, string> = {
		not_allowed: 'Your account is not authorized to access this app.',
		auth_failed: 'Authentication failed. Please try again.',
		no_code: 'No authorization code received.'
	};
</script>

<div class="flex items-center justify-center min-h-screen p-4">
	<div class="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-8 max-w-sm w-full text-center">
		<h1 class="text-2xl font-bold mb-2">TagLedger</h1>
		<p class="text-[var(--text-secondary)] text-sm mb-6">Tag-based financial reconciliation</p>

		{#if urlError}
			<div class="bg-red-500/10 border border-red-500/30 rounded px-3 py-2 mb-4 text-sm text-red-400">
				{errorMessages[urlError] ?? 'Something went wrong.'}
			</div>
		{/if}

		<a
			href="/api/auth/google"
			class="inline-flex items-center gap-3 bg-white text-gray-800 font-medium px-6 py-3 rounded-lg hover:bg-gray-100 transition-colors w-full justify-center"
		>
			<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg">
				<path d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615Z" fill="#4285F4"/>
				<path d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18Z" fill="#34A853"/>
				<path d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.997 8.997 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332Z" fill="#FBBC05"/>
				<path d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58Z" fill="#EA4335"/>
			</svg>
			Sign in with Google
		</a>

		<p class="text-xs text-[var(--text-muted)] mt-4">Only authorized accounts can access this app</p>
	</div>
</div>
